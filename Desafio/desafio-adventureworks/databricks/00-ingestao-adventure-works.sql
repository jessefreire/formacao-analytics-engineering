...
-- MAGIC
-- MAGIC Tres conferencias baratas antes de carregar 79 MB. Cada uma declara o que espera
-- MAGIC ver; se a coluna `resultado` disser `DIVERGE`, **pare** e corrija o DDL antes de
-- MAGIC seguir.
-- MAGIC
-- MAGIC 1. **As 65 tabelas nasceram?** Uma tabela faltando so apareceria no meio da carga,
-- MAGIC    depois de dezenas de comandos.
-- MAGIC 2. **Dinheiro ficou `decimal(19, 4)`?** E a conferencia mais importante desta etapa.
-- MAGIC    Com `double`, o teste de aceite do briefing sai arredondado e a causa fica
-- MAGIC    escondida no tipo, nao na consulta.
-- MAGIC 3. **Sobrou ponto flutuante em algum lugar?** A segunda olha tres colunas de uma
-- MAGIC    tabela; esta olha o schema inteiro.
